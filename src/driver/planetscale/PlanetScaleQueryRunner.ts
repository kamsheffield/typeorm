 ...
 * MySQLQueryRunner class modified to work with PlanetScale.
 */
export class PlanetScaleQueryRunner
    ...
    constructor(driver: PlanetScaleDriver, mode: ReplicationMode) {
        super()
        this.driver = driver
    ...
    protected async queryServerless(
        ...
        return new Promise(async (ok, fail) => {
            try {
                ...
                const data = await databaseConnection.execute(query, parameters)
                ...
                //console.log("=-=-=-=-=-=--= Query execution time: " + queryExecutionTime)
                ...
                result.affected = data.rowsAffected
                if (query.includes("SELECT")) {
                    result.raw = data.rows
                } else {
                    result.raw = data
                }
    ...
    stream(
        ...
        throw new TypeORMError("PlanetScale does not support streaming.")
    }
    ...
    async getSchemas(database?: string): Promise<string[]> {
        throw new TypeORMError(
            `PlanetScale driver does not support table schemas`,
        )
    }
    ...
    async getCurrentDatabase(): Promise<string> {
        // getting the database name via query in planetscale doesn't seem to work as well as it does in mysql
        // check the driver options first, then the url, then the query as a last resort
        if (this.driver.options.database) {
            return this.driver.options.database
        } else if (this.driver.options.url) {
            const url = new URL(this.driver.options.url)
        ...
        // SELECT DATABASE() doesn't work in planetscale until we USE a specific database
        // so we have to get the list of databases and filter out the system ones
        const databasesResult = await this.query(`SHOW DATABASES;`)
        const allDatabases = databasesResult.rows.map(
            (db: { [x: string]: string | any[] }) => db["Database"],
        )
        ...
        if (filteredDatabases.length > 0) {
            const database = filteredDatabases[0]
            ...
            throw new TypeORMError(`Could not determine current database`)
        }
    }
    ...
    async hasSchema(schema: string): Promise<boolean> {
        throw new TypeORMError(`PlanetScale does not support table schemas`)
    }
    ...
    async createSchema(
        ...
        throw new TypeORMError(
            `Schema create queries are not supported by PlanetScale.`,
        )
    }
    ...
    async dropSchema(schemaPath: string, ifExist?: boolean): Promise<void> {
        throw new TypeORMError(
            `Schema drop queries are not supported by PlanetScale.`,
        )
    }
    ...
    async createTable(
        ...
        createForeignKeys: boolean = false,
    ): Promise<void> {
        if (ifNotExist) {
    ...
    async createUniqueConstraint(
        ...
        throw new TypeORMError(
            `PlanetScale does not support unique constraints. Use unique index instead.`,
        )
    }
    ...
    async createCheckConstraint(
        ...
        throw new TypeORMError(
            `PlanetScale does not support check constraints.`,
        )
    }
    ...
    async createExclusionConstraint(
        ...
        throw new TypeORMError(
            `PlanetScale does not support exclusion constraints.`,
        )
    }
    ...
    async clearDatabase(database?: string): Promise<void> {
        ...
        try {
            ...
            // PlanetScale driver requires sequential execution of queries
            for (const index in dropViewQueries) {
                const dropViewQuery = dropViewQueries[index]
                await this.query(dropViewQuery["query"])
            }
            ...
            // PlanetScale driver requires sequential execution of queries
            for (const index in dropQueries) {
                const dropQuery = dropQueries[index]
                await this.query(dropQuery["query"])
            }
    ...
    protected async loadTables(tableNames?: string[]): Promise<Table[]> {
        ...
        // Original indicesQuery for Mysql below, PlanetScale doesn't support
        // joins from INFORMATION_SCHEMA
        ...
        const dbPrimaryKeys: ObjectLiteral[] = await this.query(primaryKeySql)
        const dbCollations: ObjectLiteral[] = await this.query(collationsSql)
                    ...
                    dbColumns
                        ...
                        .map(async (dbColumn) => {
                            ...
                            if (
                                ...
                                // uppercase so the comparison in PlanetScaleDriver#compareDefaultValues does not fail.
                                tableColumn.default =
                            ...
                            if (dbColumn["EXTRA"].indexOf("on update") !== -1) {
                                // New versions of MariaDB return expressions in lowercase.  We need to set it in
                                // uppercase so the comparison in PlanetScaleDriver#compareExtraValues does not fail.
